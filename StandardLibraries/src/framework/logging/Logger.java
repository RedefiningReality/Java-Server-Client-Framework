...
public class Logger
{
	...
	/**
     * Creates a new logger for logging messages to the console. This constructor 
     * sets the verbose and message level, both of which are parameters. Additionally,
	 * it accepts the parameter <code>className</code>, which sets the name of the 
     ...
     * 
     * @param verbose	Whether or not messages should be logged
     * @see #verbose
     * @see #printMessage
     * @since 3.0
     */
     ...
     * 4 => DATA
     * 
     * @param messageLevel	The log level of messages to be printed
     * @see #messageLevel
    ...
    public void printMessage(String message, int level) {
        ...
        if ((verbose && messageLevel >= level) && level != HIGHEST_PRIORITY) {
            ...
            System.out.println(formatter.format(LocalDateTime.now()) + " " + className 
            		+ ": [" + stringLevel + "] " + message);
            
            if(logFile != null)
				try {
					logFile.write(formatter.format(LocalDateTime.now()) + " " + className 
		            		+ ": [" + stringLevel + "] " + message);
					logFile.newLine();
					logFile.flush();
        ...
        if (level == ERROR) {
            // Exits the system (stops the program) with error code 'ERROR' (1)
            System.exit(ERROR);
        }
    }
}
