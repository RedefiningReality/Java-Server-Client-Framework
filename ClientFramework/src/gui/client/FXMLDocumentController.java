...
public class FXMLDocumentController implements Initializable {
    ...
    private void retrieveHostName() throws IOException {
    	...
    	if(propertiesFile) {
    		...
    		temp = new PropertiesDialog(textInput, "hostName");
    	} else {
    ...
    private void retrievePortNumber() throws IOException {
        ...
        NumericInputDialog<Integer> intInput = new NumericInputDialog<Integer>();
        ...
        if(propertiesFile) {
        	...
        	temp = new PropertiesDialog(intInput, "PORT");
        } else {
        ...
        PORT = Integer.parseInt(temp.start());
    ...
    private void connect() throws FileNotFoundException, IOException, InterruptedException {
    	...
    	if(transfers.size() > 0) {
	    	try {
				client = new ClientFramework(hostName, PORT, ClientGUI.getVerbose(), 
						ClientGUI.getMessageLevel());
			} catch (IOException ex) {
				logger.printMessage("line 122: IOException: Unable to connect to server: "
						+ ex.getMessage(), WARNING);
				
				logger.printMessage("Displaying aforementioned warning...", INFO);
				new ErrorDialog("Connection Failure", "Unable to Connect to Server.", "Please ensure that "
						+ "you have entered a valid Internet Protocol address and port number.").start();
    		...
    		logger.printMessage("Displaying aforementioned warning...", INFO);
			new WarningDialog("Transfer Selection", "No Transfers Selected", "Please ensure that you have "
    ...
    public void initialize(URL url, ResourceBundle rb) {
    	logger = new Logger("gui.client.FXMLDocumentController", ClientGUI.getVerbose(), 
    			ClientGUI.getMessageLevel());
        ...
        try {
            ...
            logger.printMessage("line 186: IOException: Unable to retrieve host name and port "
            		+ "number from properties file: " + ex.getMessage() + ": Terminating...", 
            		ERROR);
        }
